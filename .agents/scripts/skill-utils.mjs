...
export function createOpenAiYamlContent({ displayName, shortDescription, defaultPrompt, allowImplicitInvocation = true }) {
  return [
    'interface:',
...
export function replaceMarkedSection(markdown, startMarker, endMarker, content) {
...
export function renderSkillRegistryTable(entries) {
  ...
  for (const entry of entries) {
    ...
    lines.push(`| \`${entry.name}\` | ${scopeLabel(entry.scope)} | ${whenToUse} |`);
  }
...
export function upsertTableRow(markdown, sectionHeading, rowKey, rowText) {
  ...
  const rowIndex = lines.findIndex((line, index) => index >= tableStart && index < tableEnd && rowPattern.test(line));
...
export function upsertSkillReference(markdown, name, lineText) {
  ...
  const existingIndex = lines.findIndex((line, index) => index > headingIndex && index < sectionEnd && line.includes(marker));
