...
- [x] 1.2 Create `apps/api/db/migrations/00002_workflows.sql` with `workflows` table (id, tenant_id, slug, name, description, status, current_version, version, created_at, updated_at) + `UNIQUE(tenant_id, slug)` + `-- +goose Down`
- [x] 1.3 Add `workflow_versions` table (id, workflow_id FK, tenant_id, version_no, definition JSONB, status, is_current, created_at, updated_at) + `UNIQUE(workflow_id, version_no)` + indexes
...
- [x] 1.7 Verify: all tables use `id UUID PK DEFAULT gen_random_uuid()`, `created_at`, `updated_at` standard columns; snake_case; FK `ON DELETE CASCADE`; indexes on FK/tenant columns
...
- [x] 3.2 Create `internal/domain/entities/workflow.go` — `Workflow` struct + `WorkflowStatus` typed constants (DRAFT/VALIDATING/VALID/PUBLISHED/ARCHIVED)
- [x] 3.3 Create `internal/domain/entities/workflow_version.go` — `WorkflowVersion` struct + `VersionStatus` constants
...
- [x] 4.2 Create `internal/domain/repositories/workflow_repository.go` — `IWorkflowRepository` interface
- [x] 4.3 All methods accept explicit `ctx` + `tenantID string`
...
- [x] 5.2 Create `internal/infrastructure/database/pgx_workflow_repository.go` implementing `IWorkflowRepository` via sqlc `Queries`
- [x] 5.3 Constructor `NewPgxWorkflowRepository(pool *pgxpool.Pool) repositories.IWorkflowRepository` (mirrors `NewPgxAuthRepository`)
